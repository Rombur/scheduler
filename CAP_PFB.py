...
#----------------------------------------------------------------------------#
  ...
  def Create_initial_scheduling(self) :
    """Create an initial scheduling that will be improved."""
...
# TODO: the next search is very slow when the number of tasks increases
...
# Need to wait for the processor to be free
#            if proc_end_time[task.subdomain_id] > starting_time :
#              starting_time = proc_end_time[task.subdomain_id]
            starting_time = max(starting_time,proc_end_time[task.subdomain_id])
      ...
      for i in range(n_procs) :
        if len(part_schedule[i])!=0 :
          ...
          if task[0].waiting_tasks!=[] :
            for waiting_task_id in task[0].waiting_tasks :
              waiting_task = self.schedule[self.schedule_id_map[waiting_task_id]]
              ready = updated_time[self.schedule_id_map[waiting_task_id]]
              if ready==False :
                break
              if t>waiting_task[1] :
                t = waiting_task[1]
